...
| MuseScore engraving (PDF / SVG scores) | Optional: PDF and SVG come from the headless OSMD renderer (node + the frontend's node_modules); MuseScore stands in when that is missing. The engine searches `PATH` (`MuseScore4`, `MuseScore3`, `mscore`, `mscore4portable`, `musescore`), the snap, flatpak, AppImage and `/opt` locations, the path saved in Settings, and `MUSESCORE_BIN`. |
