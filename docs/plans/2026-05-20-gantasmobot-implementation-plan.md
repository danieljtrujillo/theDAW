...
Copy `C:\Users\skream\projects\SunoHarvester\src\api\key_pool.py` to `C:\Users\skream\projects\theDAW\backend\key_pool.py`.

Change `PROJECT_ROOT` to point to theDAW:
...
Remove the `_read_frontend_env` method and its references (theDAW doesn't have a `GANTASMO-SUNO-FRONTEND/.env`). In `_load_env_keys`, remove the `fe_env` logic — just use `os.environ.get(var, "")`.
...
```bash
cd C:\Users\skream\projects\theDAW
...
## Task 2: Port assistant_chat_routes.py to theDAW backend
...
Copy `C:\Users\skream\projects\SunoHarvester\src\api\assistant_chat_routes.py` to `C:\Users\skream\projects\theDAW\backend\assistant_routes.py`.
...
```python
PROJECT_CWD = r"C:\Users\skream\projects\theDAW"
```
...
if req.messages:
    ...
    if user_text:
        ...
        if rag_context:
            system_msg = ChatMessage(role="system", content=theDAW_SYSTEM_PROMPT + "\n\n" + rag_context)
            req.messages = [system_msg] + list(req.messages)
```

Add `theDAW_SYSTEM_PROMPT` constant (see Task 8 for the full prompt — for now use a placeholder):

```python
theDAW_SYSTEM_PROMPT = "You are the theDAW Assistant, an expert in Stable Audio 3 audio generation."
```
...
git commit -m "feat(assistant): mount assistant API + RAG in theDAW backend"
```
...
**ProviderModelSelector.tsx** — Copy from `C:\Users\skream\projects\SunoHarvester\GANTASMO-SUNO-FRONTEND\components\Assistant\ProviderModelSelector.tsx` into `frontend/src/orb-kit/ProviderModelSelector.tsx`. No changes needed (it uses lucide-react which theDAW already has, and Tailwind which theDAW already has).
...
2. Change default title: `title = 'theDAW Assistant'`
...
git commit -m "feat(assistant): port orb-kit UI components to theDAW"
```
...
## Task 7: Write the theDAW system prompt
...
Replace `theDAW_SYSTEM_PROMPT` in `backend/assistant_routes.py` with:
...
```bash
git add backend/assistant_routes.py
git commit -m "feat(assistant): add comprehensive theDAW system prompt"
```
...
git commit -m "feat(assistant): mount GANTASMOB0T orb in theDAW UI"
```
