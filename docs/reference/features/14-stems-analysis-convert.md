...
- **From an EDIT clip.** Separation is keyed on library entries, but a timeline clip can reach it: the clip context menu's **Separate Stems → Tracks…** bridges the clip's in-memory blob into the entry-keyed API via `importEntry`, writes the resulting entry id back onto the clip so a re-run hits the cache, and explodes each returned stem onto its own colour-coded track at the source clip's position and trim, muting the source (`WaveformEditor.tsx`). See [05-edit.md](05-edit.md).
- **Sidecar venv health.** The sidecar runs from `integration-package/backend/.sidecar_venv`. Because that venv's `pyvenv.cfg` records an absolute path to its base Python, a moved or renamed user profile makes every separation fail with a dependency-install error that names `demucs`/`torch` even though the packages are present. `GET /api/stems/probe` reports the real cause in `demucs_error`; the fix is to point `home` at an installed Python of the same minor version, not to reinstall.
