...
### Runs offline?
Yes, effectively all of it. DRAW, arpeggiator, sheet import, the Vocal Engine, and audio-to-MIDI run locally (model checkpoints for basic-pitch/piano-transcription/whisper download once, then cache and run CPU-only). PDF/SVG need node and the frontend's dependencies (the headless OSMD renderer) or a local MuseScore install, and degrade to MusicXML/ABC when both are missing. The only true cloud call is the optional Gemini vocal-cleanup, which needs `GEMINI_API_KEY` on the server.
