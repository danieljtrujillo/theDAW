...
Returns one entry per configured provider. `claude` — Better Claude Code by skreamb0t (@StarskreamEXE), the provider that drives the local Claude Code CLI — is always present with `has_key: true`. Remote providers with no API key in the environment are still listed, and requests to them fail until a key is added.
