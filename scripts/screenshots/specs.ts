...
export const FEATURE_DESCRIPTORS: FeatureDescriptor[] = [
  ...
  // --- Added 2026-09 after the docs sweep found these shipped but uninventoried.
  // Without entries here the coverage report reads 100% while real gaps exist.
  {
    id: 'perform-sway-deck-scenes-punches',
    name: 'PERFORM clip grid driven by the SwayCommand deck: pads launch scenes or punch FX, routing travels in the .tasmo',
    domain: 'daw',
    sourcePaths: ['frontend/src/components/session/DawSessionGrid.tsx', 'frontend/src/components/session/SwayDeck.tsx', 'frontend/src/state/performRouting.ts', 'frontend/src/lib/tasmoToSession.ts'],
    evidence: ['isNote ccMods: note-on -> max, note-off -> min', 'latch', 'DECK_FACTORY knobs 20-27 / xy 50,38 / pads 24-39', 'perform_routing hydrate'],
    status: 'implemented',
    docSearchTerms: ['SwayCommand deck', 'punch', 'scene launches', 'live effect chain'],
  },
  {
    id: 'rack-kargyraa-sub',
    name: 'Kargyraa Sub rack effect: octave-divider worklet, growl gate, morphing vowel formants, sygyt whistle band',
    domain: 'edit',
    sourcePaths: ['frontend/src/lib/rackEffects.ts', 'frontend/public/subharmonic.worklet.js'],
    evidence: ["id: 'kargyraa'", 'subharmonic-processor', 'KARGYRAA_VOWELS'],
    status: 'implemented',
    docSearchTerms: ['Kargyraa Sub', 'subharmonic', 'growl', 'whistle'],
  },
  {
    id: 'edit-effect-windows-unified',
    name: 'One FX chain list per track/master with a floating control window per entry (built-in, VST3 and .gan alike)',
    domain: 'edit',
    sourcePaths: ['frontend/src/components/audio/EffectWindows.tsx', 'frontend/src/components/audio/WaveformEditor.tsx'],
    evidence: ['EffectWindowsHost', 'FxChainList', 'entryKind vst | gan | fx'],
    status: 'implemented',
    docSearchTerms: ['floating control window', 'FX button', 'chain list', 'one concept'],
  },
  {
    id: 'edit-autosave-crash-recovery',
    name: 'EDIT autosave to a content-addressed OPFS asset layer with a recovery offer at startup',
    domain: 'edit',
    sourcePaths: ['frontend/src/lib/editorAutosave.ts', 'frontend/src/components/layout/AutosaveRecoveryNotice.tsx'],
    evidence: ['assets/<sha256>.bin', 'debounced manifest', 'saving stays paused until answered'],
    status: 'implemented',
    docSearchTerms: ['Autosave and crash recovery', 'OPFS', 'restore', 'recover'],
  },
  {
    id: 'nodefi-node-graph',
    name: 'NodeF.I. (NODEFI) node-graph pipelines: canvas, palette, inspector, templates, LIVE, saved sets',
    domain: 'daw',
    sourcePaths: ['frontend/src/views/NodefiView.tsx', 'frontend/src/state/nodefiStore.ts', 'frontend/src/lib/nodefiRunner.ts', 'frontend/src/state/nodefiSetsStore.ts'],
    evidence: ['NodefiView', 'thedaw-nodefi', '.nodefi.json', 'NODEFI_TEMPLATES'],
    status: 'implemented',
    docSearchTerms: ['NodeF.I.', 'NODEFI', 'node-graph', 'pipeline'],
  },
  {
    id: 'assistant-editor-tools-approval-tiers',
    name: 'Assistant editor_* tool vocabulary with T1/T2 approval tiers and a confirmation card for destructive or GPU-spending actions',
    domain: 'assistant',
    sourcePaths: ['backend/assistant_routes.py', 'frontend/src/orb-kit/tool-tiers.ts', 'frontend/src/orb-kit/AssistantPanel.tsx', 'frontend/src/state/appUiStore.ts'],
    evidence: ['editor_get_state', 'editor_split_clip', 'pendingAction', 'navigateTo'],
    status: 'implemented',
    docSearchTerms: ['assistant can reach every workspace', 'confirmation', 'Library rail', 'honest failure'],
  },
  {
    id: 'score-follow-note-by-note',
    name: 'Score panel follows playback note by note: gliding staff, highlighted notehead, alphaTab beat cursor with Follow toggle',
    domain: 'daw',
    sourcePaths: ['frontend/src/components/layout/ScoreView.tsx', 'backend/modules/notation/arrangers/guitar_tab.py'],
    evidence: ['keepCursorVisible', 'applyNoteHighlight', 'external-media player mode', 'tempo 120'],
    status: 'implemented',
    docSearchTerms: ['Follow', 'notehead', 'beat cursor', 'tabs'],
  },
  {
    id: 'swayproj-import',
    name: 'Audima Sway .swayproj import: presets and zones parsed, PERFORM gesture bindings seeded by preset name',
    domain: 'daw',
    sourcePaths: ['backend/modules/dawimport/swayproj.py', 'backend/modules/dawimport/router.py', 'frontend/src/lib/fileFilters.ts'],
    evidence: ['POST /api/dawimport/sway', 'magic FF 02', 'zero tracks by design'],
    status: 'implemented',
    docSearchTerms: ['.swayproj', 'Audima Sway', 'preset', 'DAW import'],
  },
  {
    id: 'linux-setup-and-launcher',
    name: 'Linux support: theDAW.sh launcher, cu126 torch via uv sources, pyk4a glibc caveat, docs/linux/setup-guide.md',
    domain: 'settings',
    sourcePaths: ['theDAW.sh', 'backend/_devstack.py', 'pyproject.toml', 'docs/linux/setup-guide.md'],
    evidence: ['shlex.split off Windows', "sys_platform == 'linux'", '--no-install-package pyk4a-bundle'],
    status: 'implemented',
    docSearchTerms: ['theDAW.sh', 'Linux', 'pyk4a-bundle', 'uv sync'],
  },
  {
    id: 'hf-signin-gated-download',
    name: 'Hugging Face sign-in card raised in-app when a gated model download is refused; token stored where the HF CLI reads it',
    domain: 'library',
    sourcePaths: ['frontend/src/components/layout/DownloadDock.tsx', 'frontend/src/notices/FeatureGateNotices.tsx', 'backend/modules/hfauth/router.py'],
    evidence: ["requireFeature({ id: 'hf:gated-model'", 'POST /api/hfauth/login', "kind === 'gated'"],
    status: 'implemented',
    docSearchTerms: ['Hugging Face sign-in', 'gated', 'read token', 'sign-in card'],
  },
  {
    id: 'underfit-venv-self-repair',
    name: 'Underfit environment self-repair: half-built venv detected from site-packages, Repair button rebuilds it, live uv sync output, real port',
    domain: 'train',
    sourcePaths: ['backend/modules/underfit/sidecar.py', 'frontend/src/views/UnderfitView.tsx'],
    evidence: ['missing_venv_packages', 'venv is incomplete', 'Repair environment', 'log_tail'],
    status: 'implemented',
    docSearchTerms: ['Repair environment', 'Create environment', 'incomplete', 'uv sync'],
  },
];
