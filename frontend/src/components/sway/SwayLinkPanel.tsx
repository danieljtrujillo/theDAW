/**
 * SWAY LINK panel -- theDAW's own half of the Sway integration, beside the
 * embedded SwayCommand cockpit in the SWAY tab.
 *
 * Four things live here, none of which belong to SwayCommand because theDAW
 * consumes them itself:
 *   - The six expressive dimensions: arm LEARN (binds the dimension to the next
 *     CC the controller sends), show the bound CC and a live 0..1 meter, and
 *     route it to a BindableTarget. This is the ONLY caller of the sway bus's
 *     startLearn, and Perform's routing panel reads what it binds.
 *   - The VJ's six camera-pose channels and their routes.
 *   - The DAW-control surface: mirrors the Sway's Ableton Remote Script map
 *     (transport, 8 volume + 8 pan CCs, 16 pads) onto theDAW's EDIT timeline.
 *     Its toggle is also the arbitration point when SwayCommand wants the same
 *     physical pads.
 *   - Controller bindings recovered from an imported .tasmo/.als project.
 *
...
export const SwayLinkPanel: React.FC = () => {
